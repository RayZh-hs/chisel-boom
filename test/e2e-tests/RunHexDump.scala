...
object RunHexDump extends App {
    ...
    def convertByteToWordAligned(inputPath: Path): Path = {
        ...
        val outputPath = inputPath.getParent.resolve(
          inputPath.getFileName.toString.replace(".hex", "_word_aligned.hex")
        )
        val writer = Files.newBufferedWriter(outputPath)
        for (line <- lines) {
            if (
              line.trim.nonEmpty && !line
                  .startsWith("#") && !line.startsWith("@")
            ) {
                val tokens = line.trim.split("\\s+")
                if (
                  tokens.exists(token =>
                      token.length != 2 && !token.matches("[0-9a-fA-F]+")
                  )
                ) {
                    throw new Exception(
                      s"Invalid hex data format in line: $line"
                    )
                }
                // squeeze 4 bytes into one word
                val wordTokens = tokens
                    .grouped(4)
                    .map { byteGroup =>
                        val paddedGroup = byteGroup.padTo(
                          4,
                          "00"
