...
The functional unit of ALQ is the Arithmetic Logic Unit (ALU).

### Branch Queue (BRQ)

Originally part of the Integer Issue Queue (IIQ) in BOOM, the BRQ has been separated to handle all branch and comparison operations since in the simulation system the small instruction window size means and low memory latency means that stress will be placed on IIQ entries.

The functional unit of BRQ is the Branch Unit (BRU).
...
The functional unit of LSQ is the Load Store Unit (LSU).
