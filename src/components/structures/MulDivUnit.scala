...
class MulDivUnit extends Module {
    ...
    // Configuration
    val mulLatency =
        Configurables.MUL_LATENCY // Number of pipeline stages for Multiply

    // State Definitions
    val sIdle = 0.U(3.W)
    val sMul = 1.U(3.W) // Used for MUL
    val sDiv = 2.U(3.W) // Used for DIV
    val sDone = 3.U(3.W)
    ...
    // Division Helper Logic (Combinational)
    val is_signed_div =
    ...
    // Chisel will generate a Wallace Tree multiplier here, occupying the two cycles
    ...
    val mul_res =
        Mux(rFn === MduFunc.MUL, mul_pipe_2(31, 0), mul_pipe_2(63, 32))

    // Finite State Machine
    // IDLE -> MUL / DIV -> DONE (send result and return to IDLE)
    // TODO: Use enum for this state machine; too messy now
    switch(state) {
        is(sIdle) {
            io.req.ready := true.B
        ...
        is(sDiv) {
            ...
            when(cnt === 1.U) {
                ...
                val rawResult = Mux(
                  rFn === MduFunc.DIV || rFn === MduFunc.DIVU,
